...
/** Localized closing line for Pinterest's data-pin-description, used when no
 *  per-artwork description text is available (~4.3k artworks mid-enrichment). */
const PIN_DESCRIPTION_SUFFIX: Record<string, string> = {
  en: "Free high-resolution download — public domain, no attribution required.",
  es: "Descarga gratuita en alta resolución — dominio público, sin atribución necesaria.",
  pt: "Download gratuito em alta resolução — domínio público, sem necessidade de atribuição.",
  de: "Kostenloser Download in hoher Auflösung — gemeinfrei, keine Namensnennung erforderlich.",
  fr: "Téléchargement gratuit en haute résolution — domaine public, aucune attribution requise.",
  it: "Download gratuito in alta risoluzione — dominio pubblico, nessuna attribuzione richiesta.",
  ja: "無料で高解像度ダウンロード可能 — パブリックドメイン、著作権表示は不要です。",
  ko: "무료 고해상도 다운로드 — 퍼블릭 도메인, 저작자 표시가 필요하지 않습니다.",
  ru: "Бесплатная загрузка в высоком разрешении — общественное достояние, указание авторства не требуется.",
  zh: "免费高分辨率下载 — 公共领域作品，无需署名。",
};

/** Strip markdown emphasis/quote artifacts and collapse whitespace, for
 *  attributes (Pinterest description) that must be plain text. */
...
/** Builds the value for an artwork image's `data-pin-description` attribute:
 *  "{title} — {artist}. {first sentence of description, if any}. {localized suffix}",
 *  capped at 480 chars (Pinterest's practical limit) without mid-word truncation. */
export function buildPinDescription(
  ...
  const suffix = PIN_DESCRIPTION_SUFFIX[locale] ?? PIN_DESCRIPTION_SUFFIX.en;
  let text = artist ? `${title} — ${artist}.` : `${title}.`;

  if (description?.trim()) {
    const cleaned = stripMarkdownArtifacts(description);
    const firstSentence = cleaned.split(/(?<=[.!?])\s/)[0]?.trim();
    if (firstSentence) {
      text += ` ${firstSentence}${/[.!?]$/.test(firstSentence) ? "" : "."}`;
    }
  }
  ...
  const cut = text.slice(0, 479);
  const lastSpace = cut.lastIndexOf(" ");
  return `${cut.slice(0, lastSpace > 400 ? lastSpace : 479)}…`;
}

type ArtworkPinSource = ArtworkImageSource & {
  title: string;
  artist_display?: string | null;
  description?: string | null;
};

/** Full set of Pinterest attributes for one artwork image: the canonical page
 *  URL, the largest available image (never a small grid thumbnail), and a
 *  localized description. `canonicalUrl` is the absolute URL of the specific
 *  locale page this image appears on. */
export function buildArtworkPinAttrs(
  artwork: ArtworkPinSource,
  locale: string,
  canonicalUrl: string
): { "data-pin-url": string; "data-pin-media": string; "data-pin-description": string } {
  const media = artworkOriginalUrl(artwork) || artworkDetailImageUrl(artwork);
  return {
    "data-pin-url": canonicalUrl,
    "data-pin-media": media,
    "data-pin-description": buildPinDescription(locale, artwork.title, artwork.artist_display, artwork.description),
  };
}
