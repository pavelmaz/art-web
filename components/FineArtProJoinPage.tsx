...
export async function FineArtProJoinPage({ locale, searchParams }: FineArtProJoinPageProps) {
  ...
  const planCopy =
    plan === "yearly"
      ? { name: c.yearlyPlan, price: c.yearlyPrice, billing: c.yearlyBilling, save: c.yearlySave }
      : plan === "monthly"
        ? { name: c.monthlyPlan, price: c.monthlyPrice, billing: c.monthlyBilling, save: null }
        : null;
      ...
      <div className="mx-auto grid max-w-5xl gap-8 lg:grid-cols-[minmax(0,1fr)_400px] lg:items-start lg:gap-14">
        ...
        <div className="mt-5 flex items-center gap-2 text-xs">
          ...
          <span className="inline-flex items-center gap-1.5 rounded-full border border-[#d8d5cd] px-2.5 py-1 text-[#6b6b6b]">
            <span className="grid size-4 place-items-center rounded-full bg-[#e8e6e1] text-[10px]">2</span>
            {c.joinStepPayment}
          </span>
        </div>
        ...
        {planCopy ? (
          <div className="mt-5 rounded-xl border border-[#e3e0d9] bg-white p-4">
            ...
            <div className="mt-1.5 flex items-baseline justify-between gap-3">
              <span className="text-sm font-semibold text-[#1a1a1a]">{planCopy.name}</span>
              <span className="text-lg font-bold tracking-tight text-[#1a1a1a]">{planCopy.price}</span>
            </div>
            <div className="mt-1 flex items-center justify-between gap-3">
              <span className="text-xs text-[#6b6b6b]">{planCopy.billing}</span>
              {planCopy.save ? (
        ...
        <p className="mt-4 text-sm leading-relaxed text-[#4a4a4a]">{c.joinWhyAccount}</p>
        ...
        <div className="mt-6 rounded-xl border border-[#e8e6e1] bg-[#faf9f6] p-4">
          ...
          <ul className="mt-2 space-y-1.5">
            {c.comparison.map((row) => (
              <li key={row.feature} className="flex items-start gap-2 text-[13px] text-[#4a4a4a]">
                <svg viewBox="0 0 20 20" fill="none" className="mt-0.5 size-4 shrink-0" aria-hidden>
                  <path d="M4.5 10.5l3.5 3.5 7.5-7.5" stroke="#3b8e3f" strokeWidth="2.2"
