  ...
  const jsonLd = {
    ...
    itemListElement: [
      ...
      },
      {
        ...
        item: category ? `${siteUrl}${category.href}` : `${siteUrl}/fr/œuvres`,
      },
      {
        "@type": "ListItem",
        position: 3,
        name: artwork.title,
        item: `${siteUrl}/fr/œuvres/${artwork.slug}`,
      },
    ],
  };
...
export default async function ArtworkDetailPageFr({ params }: ArtworkPageProps) {
  ...
  const breadcrumbItems = [
    ...
    ...(artwork.artist_display?.trim()
      ? [{ label: artwork.artist_display.trim(), href: `/fr/artistes/${slugify(artwork.artist_display)}` }]
      : []),
    { label: artwork.title },
  ];
        ...
        <div className="flex flex-col gap-8 lg:flex-row lg:items-start">
          <ArtworkInsightsProvider artwork={artwork} locale="fr">
            <div className="flex-1 space-y-4">
              ...
              <Breadcrumbs items={breadcrumbItems} currentPath={`/fr/œuvres/${artwork.slug}`} includeJsonLd={false} />
            </div>
          </ArtworkInsightsProvider>
          ...
          <aside className="w-full lg:w-80">
            <div className="space-y-4 rounded-2xl bg-[#f5f5f5] p-5 lg:sticky lg:top-6">
              <div>
                ...
                {artistSlug ? (
                  <Link href={`/fr/artistes/${artistSlug}`} className="text-sm text-[#6b6b6b] hover:text-[#1a1a1a]">
                    {artist}
                  </Link>
                ) : (
              ...
              <div className="space-y-3 rounded-lg bg-[#eceff3] p-3">
                ...
                {artwork.museum?.trim() ? (
                  <div>
                    ...
                    <Link
                      href={`/fr/musées/${slugify(artwork.museum)}`}
        ...
        {descriptionText?.trim() ? (
          <section className="mt-10">
            ...
            {artistSlug && relatedArtworks.length === 0 ? (
              <SectionCtaLink href={`/fr/artistes/${artistSlug}`}>{t.browseAll}</SectionCtaLink>
            ) : null}
          </section>
        ) : null}
          ...
          <section className="mt-10">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
              {relatedArtworks.map((item) => (
                <a key={item.id} href={`/fr/œuvres/${item.slug}`} className="group block">
