...
export const metadata: Metadata = {
  title: "Fine Art Free — Téléchargez 80 000+ Œuvres d'art du Domaine Public",
  description:
    "Explorez et téléchargez 80 000+ peintures classiques gratuitement. Art du domaine public des plus grands musées du monde. Gratuit pour usage personnel et commercial.",
  openGraph: {
    title: "Fine Art Free — Téléchargez 80 000+ Œuvres d'art du Domaine Public",
    description:
      "Explorez et téléchargez 80 000+ peintures classiques gratuitement. Art du domaine public des plus grands musées du monde. Gratuit pour usage personnel et commercial.",
  ...
  alternates: {
    canonical: "https://fineartfree.com/fr",
  ...
  const genreImages = await Promise.all(
    browseGenres.map(async (genre) => {
      const { data } = await supabase
        ...
        .limit(1);
      const row = data?.[0];
      const slug = getGenreSlugForLocale(genre, "fr");
      const label = getGenreLabelForLocale(genre, "fr");
      return {
        slug,
        label,
      ...
      <div className="w-full overflow-x-auto scrollbar-hide bg-[#f6f4ee] px-5 py-6">
        <div className="flex gap-3 min-w-max">
          {genreImages.map((genre) => (
            <Link
              key={genre.slug}
              href={`${localePath("fr", "genres")}/${genre.slug}`}
            ...
            <div className="flex gap-3 overflow-x-auto pb-2">
              {safeFeaturedArtists.map((artist) => {
                ...
                return (
                  <Link
                    key={artist.name}
                    href={`/fr/artists/${slugify(artist.name)}`}
