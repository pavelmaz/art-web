...
async function resolveGenreHubLinkLocal(
  ...
  const { data, error } = await supabase
    .from("genres")
    .select("name, name_fr, slug, slug_fr")
    .eq("name", name)
    .maybeSingle();
  ...
  const linkSlug = row.slug_fr?.trim() || row.slug;
  const label = row.name_fr?.trim() || row.name;
  return { href: `/fr/genres/${linkSlug}`, label };
}
...
async function resolveStyleHubLinkLocal(
  ...
  const linkSlug = row.slug_fr?.trim() || row.slug;
  const label = row.name_fr?.trim() || row.name;
  return { href: `/fr/styles/${linkSlug}`, label };
}
...
async function resolveCategoryBreadcrumbFr(
  artwork: ArtworkRow
  ...
  const jsonLd = {
    ...
    itemListElement: [
      ...
      },
      {
        "@type": "ListItem",
        position: 2,
        name: category?.label || "Œuvres",
        item: category ? `${siteUrl}${category.href}` : `${siteUrl}/fr/artworks`,
      },
      {
        "@type": "ListItem",
        position: 3,
        name: artwork.title,
        item: `${siteUrl}/fr/artworks/${artwork.slug}`,
      },
    ],
  };
...
async function getArtworkBySlug(slug: string): Promise<ArtworkRow | null> {
  const selectColumns = artworkDetailSelectColumns("fr");
  ...
  if (!primary.error && primary.data) {
    ...
    row.title = row.title_fr?.trim() || row.title;
    return row;
  }
...
export async function generateMetadata({ params }: ArtworkPageProps): Promise<Metadata> {
  ...
  const artist = artwork.artist_display ?? "Artiste inconnu";
  ...
  const description =
    ...
    artwork.description_fr?.slice(0, 200) ||
    `${artwork.title} de ${artist}. Art du domaine public gratuit.`;
  ...
  return {
    ...
    description,
    alternates: {
      canonical: `https://fineartfree.com${artworkDetailPath("fr", slug)}`,
...
export default async function ArtworkDetailPageFr({ params }: ArtworkPageProps) {
  const { slug } = await params;
  const artwork = await getArtworkBySlug(slug);
  const t = getT("fr");
  ...
  const breadcrumbItems = [
    ...
    ...(artwork.artist_display?.trim()
      ? [{ label: artwork.artist_display.trim(), href: `/fr/artists/${slugify(artwork.artist_display)}` }]
      : []),
    { label: artwork.title },
  ];
  ...
  if (artwork.artist_display?.trim()) {
    const relatedQuery = await supabase
      .from("artworks")
      .select("id, title, title_fr, slug, artist_display, image_id, url, museum, style_title, genre_title, score, alt_text")
      .eq("artist_display", artwork.artist_display)
  ...
  if (artwork.genre_title) {
    const { data: genreData } = await supabase
      .from("artworks")
      .select("id, title, title_fr, slug, artist_display, image_id, url, museum, style_title, genre_title, alt_text")
  ...
  return (
    <article className="py-8">
      ...
      <div className="mx-auto max-w-7xl px-5">
        <ArtworkJsonLd artwork={{ ...artwork, description: artwork.description_fr || artwork.description }} pageUrl={`https://fineartfree.com${artworkDetailPath("fr", artwork.slug)}`} inLanguage="fr" />
          ...
          <aside className="order-2 w-full lg:w-96">
            <div className="glass-surface space-y-4 rounded-2xl p-5 lg:sticky lg:top-6">
              <div>
                ...
                <ArtistChip
                  name={artist}
                  href={artistSlug ? `/fr/artists/${artistSlug}` : null}
              ...
              <div className="mt-4">
                <ArtworkActions artworkId={artwork.id} locale="fr" />
              </div>
              ...
              <div className="space-y-3">
                ...
                <ProDownloadRow locale="fr" downloadHref={maxDownloadHref} filename={artwork.slug} glass maxDims={artworkMaxSpecs(artwork)} maxSize={artworkMaxSize(artwork)} />
              </div>
                  ...
                  <div className="mt-3 rounded-lg bg-white p-4">
                    <p className="leading-relaxed text-[#4a4a4a]">
                      {artistDeathYear != null ? (
                        <>
                          L&apos;artiste est décédé en {artistDeathYear}, donc cette œuvre est dans le domaine public
                          dans son pays d&apos;origine et dans d&apos;autres pays où la durée des droits d&apos;auteur est la
                          vie de l&apos;artiste plus 70 ans ou moins.
                        </>
                      ) : (
                        <>
                          Cette œuvre est dans le domaine public dans son pays d&apos;origine et dans d&apos;autres pays où
                          la durée des droits d&apos;auteur est la vie de l&apos;artiste plus 70 ans ou moins.
                        </>
                      )}
                    </p>
                  </div>
              ...
              <div className="space-y-3 rounded-lg glass-inset p-3">
                ...
                {artwork.medium_display?.trim() ? (
                  <div>
                    ...
                    <p className="text-sm text-[#1a1a1a]">{localizeMedium(artwork.medium_display, "fr")}</p>
                  </div>
                ) : null}
                ...
                {artwork.museum?.trim() ? (
                  <div>
                    ...
                    <Link
                      href={`/fr/museums/${slugify(artwork.museum)}`}
                      className="text-sm text-[#1a1a1a] underline"
                    >
                      {artwork.museum}
                    </Link>
              ...
              <Link
                href="/fr/images-libres-de-droits-usage-commercial"
                    ...
                    <div className="flex justify-center">
                      <div className="relative w-fit max-w-full">
                        <ArtworkZoomImage
                          ...
                          alt={localeTranslation?.alt_text || localizeAltText(artwork, "fr")}
                        />
                        <ArtworkInsightsOverlay />
                      </div>
                    </div>
