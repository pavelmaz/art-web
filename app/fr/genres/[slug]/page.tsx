...
export async function generateMetadata({ params, searchParams }: GenrePageProps): Promise<Metadata> {
  ...
  const title = `${displayName} — Peintures gratuites | Fine Art Free`;
  const description =
    genre.description_fr?.trim() ||
  ...
  return {
    ...
    description,
    alternates: paginatedAlternates(`${localePath("fr", "genres")}/${linkSlug}`, page, buildGenreLanguageAlternates(genre.slug)),
...
export default async function GenrePage({ params, searchParams }: GenrePageProps) {
  ...
  const displayName = genre.name_fr?.trim() || genre.name;
  const intro = genre.description_fr?.trim() || genre.description?.trim() || null;
  const linkSlug = genre.slug_fr?.trim() || genre.slug;
  ...
  if (error) {
    console.error("[GenrePage/fr]", slug, genre.name, error);
  ...
  return (
    <div className="space-y-6 px-5">
      <CollectionPageJsonLd
        name={`Peintures de ${displayName}`}
        path={`/fr/genres/${linkSlug}`}
        description={intro}
      ...
      <Breadcrumbs
        items={[
          { label: "Home", href: "/fr" },
          { label: "Genres", href: "/fr/genres" },
          { label: displayName },
        ]}
        currentPath={`/fr/genres/${linkSlug}`}
      />
      <h1 className="text-3xl font-bold tracking-tight">Peintures de {displayName}</h1>
      {intro ? (
        <div className="max-w-3xl mb-8 text-sm leading-relaxed text-[#4a4a4a]">
          <p>{intro}</p>
        </div>
      ) : null}
      <ArtworkGrid artworks={uniqueArtworks} basePath="/fr" />
      <Pagination
        currentPage={page}
        totalPages={pagesOrNotFound(page, totalCount || artworks.length)}
        basePath={`/fr/genres/${linkSlug}`}
      />
    </div>
  );
}
