...
export async function generateMetadata({ params, searchParams }: StylePageProps): Promise<Metadata> {
  ...
  const displayName = style?.name_fr?.trim() || englishName;
  ...
  const title = `${displayName} — Styles gratuits | Fine Art Free`;
  const description =
    style?.description_fr?.trim() ||
    style?.description?.trim() ||
    `Découvrez ${totalCount} œuvres de ${displayName} en haute résolution. Domaine public, gratuit.`;

  const linkSlug = style?.slug_fr?.trim() || style?.slug || slug;

  return {
    title: { absolute: title },
    description,
    alternates: paginatedAlternates(`${localePath("fr", "styles")}/${linkSlug}`, page, buildStyleLanguageAlternates(style?.slug || slug)),
...
export default async function StylePage({ params, searchParams }: StylePageProps) {
  ...
  const englishName = style?.name ?? unslugifyStyle(slug);
  const displayName = style?.name_fr?.trim() || englishName;
  const intro = style?.description_fr?.trim() || style?.description?.trim() || null;
  const linkSlug = style?.slug_fr?.trim() || style?.slug || slug;
  ...
  if (error) {
    console.error("[StylePage/fr]", slug, englishName, error);
  ...
  return (
    <div className="space-y-6 px-5">
      ...
      <Breadcrumbs
        items={[
          { label: "Home", href: "/fr" },
          { label: "Styles", href: "/fr/styles" },
          { label: displayName },
        ]}
        currentPath={`/fr/styles/${linkSlug}`}
      />
      <h1 className="text-3xl font-bold tracking-tight">Style {displayName}</h1>
      {intro ? (
