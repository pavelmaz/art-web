...
export default async function SearchPage({ searchParams }: SearchPageProps) {
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-8 px-5 py-10">
      <SearchAnalytics query={q} locale="fr" results={artworks.length} />
      <div className="space-y-2">
        <h1 className="text-3xl font-semibold text-[#1a1a1a]">Résultats pour &quot;{q}&quot;</h1>
      </div>

      <section className="space-y-4">
        <h2 className="text-xl font-semibold text-[#1a1a1a]">Œuvres ({artworks.length})</h2>
        {artworks.length ? (
        ...
        <section className="space-y-4">
          ...
          <div className="flex flex-wrap gap-2">
            {matchingTags.map((tag) => (
              <Link
                key={tag}
                href={`/fr/artworks?tag=${encodeURIComponent(tag)}`}
