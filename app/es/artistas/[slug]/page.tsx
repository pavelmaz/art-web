...
export async function generateMetadata({ params }: ArtistPageProps): Promise<Metadata> {
  ...
  const title = `${artist.name} — Obras Completas Gratis para Descargar | Fine Art Free`;
  const description = `Descarga ${totalCount} obras de ${artist.name}. Pinturas de dominio público en alta resolución, gratis para cualquier uso.`;

  return {
    title,
    description,
    alternates: {
      canonical: absoluteUrl(`/es/artistas/${slug}`),
      languages: {
        en: absoluteUrl(`/artists/${slug}`),
        es: absoluteUrl(`/es/artistas/${slug}`),
        pt: absoluteUrl(`/pt/artistas/${slug}`),
      },
    },
...
export default async function ArtistPage({ params, searchParams }: ArtistPageProps) {
  ...
  if (error) {
    console.error("[ArtistPage/es]", slug, artistName, error);
  ...
  const bio = getArtistBioForLocale(artist, "es");
  ...
  return (
    <div className="space-y-8 px-5">
      ...
      <Breadcrumbs
        items={[{ label: "Inicio", href: "/es" }, { label: "Artistas", href: "/es/artistas" }, { label: artistName }]}
        currentPath={`/es/artistas/${slug}`}
      />
      <ArtistProfileHeader
        name={artistName}
