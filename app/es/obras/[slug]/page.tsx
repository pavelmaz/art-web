...
async function getArtworkBySlug(slug: string): Promise<ArtworkRow | null> {
  const selectColumns =
    "id, slug, title, artist_display, url, image_id, museum, style_title, genre_title, medium_display, date_display, dimensions, description, description_sp";
          ...
          <aside className="w-full lg:w-80">
            <div className="space-y-4 rounded-2xl bg-[#f5f5f5] p-5 lg:sticky lg:top-6">
              <div>
                ...
                {artistSlug ? (
                  <Link href={`/es/artistas/${artistSlug}`} className="text-sm text-[#6b6b6b] hover:text-[#1a1a1a]">
                    {artist}
                  </Link>
                ) : (
