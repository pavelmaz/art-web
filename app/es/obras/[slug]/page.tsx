...
async function getArtworkBySlug(slug: string): Promise<ArtworkRow | null> {
  const selectColumns =
    "id, slug, title, artist_display, url, image_id, museum, style_title, genre_title, medium_display, date_display, dimensions, description, description_sp, title_sp, death_year, img_width, img_height, orig_bytes, std_bytes";
  ...
  if (!primary.error && primary.data) {
    ...
    row.title = row.title_sp?.trim() || row.title;
    return row;
  }
...
export default async function ArtworkDetailPageEs({ params }: ArtworkPageProps) {
  ...
  if (artwork.genre_title) {
    const { data: genreData } = await supabase
      .from("artworks")
      .select("id, title, title_sp, slug, artist_display, image_id, url, museum, style_title, genre_title, alt_text")
        ...
        <div className="flex flex-col gap-8 lg:flex-row lg:items-start">
          <aside className="order-2 w-full lg:w-96">
            <div className="glass-surface space-y-4 rounded-2xl p-5 lg:sticky lg:top-6">
              ...
              <div className="mt-4">
                <ArtworkActions artworkId={artwork.id} locale="es" />
              </div>
              ...
              <div className="space-y-3">
                ...
                <ProDownloadRow locale="es" downloadHref={maxDownloadHref} filename={artwork.slug} glass maxDims={artworkMaxSpecs(artwork)} maxSize={artworkMaxSize(artwork)} />
              </div>
              ...
              <Link
                href="/es/imagenes-de-dominio-publico-para-uso-comercial"
              ...
              >
                ...
                Imágenes de dominio público para uso comercial
              </Link>
            </div>
          </aside>

          <ArtworkInsightsProvider artwork={artwork} locale="es">
