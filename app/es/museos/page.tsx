...
export const metadata: Metadata = {
  ...
  alternates: {
    canonical: absoluteUrl("/es/museos"),
...
export default async function MuseumsPage({ searchParams }: MuseumsPageProps) {
  ...
  const items = aggregated.map((row) => ({
    name: row.display,
    href: `/es/museos/${slugify(row.display)}`,
    count: row.count,
  ...
  return (
    <div className="space-y-8 px-5">
      <div>
        ...
        <p className="mb-8 text-sm text-[#6b6b6b]">Explorar obras de arte por museo</p>
      </div>

      {paginated.length ? <BrowseHubGrid items={paginated} /> : <p className="text-sm text-[#6b6b6b]">No se encontraron museos.</p>}

      <Pagination currentPage={page} totalPages={totalPages} basePath="/es/museos" />
    </div>
  );
}
