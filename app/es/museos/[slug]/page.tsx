...
export async function generateMetadata({ params }: MuseumPageProps): Promise<Metadata> {
  ...
  const description = `Explora ${totalCount} obras de arte de ${museumName} gratis para descargar. Arte de dominio público en alta resolución para cualquier uso.`;
...
export default async function MuseumPage({ params, searchParams }: MuseumPageProps) {
  ...
  return (
    <div className="space-y-6 px-5">
      <Breadcrumbs
        items={[{ label: "Inicio", href: "/es" }, { label: "Museos", href: "/es/museos" }, { label: museumName }]}
        currentPath={`/es/museos/${slug}`}
      />
      <h1 className="text-3xl font-bold tracking-tight">Obras de {museumName}</h1>
      <p className="max-w-3xl text-neutral-700">
        Explora obras de arte y pinturas de dominio público de {museumName}, gratis para descargar.
      </p>
