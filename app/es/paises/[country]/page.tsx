...
export default async function CountryPageEs({ params, searchParams }: CountryPageProps) {
  ...
  if (error) {
    console.error("[CountryPage/es]", slug, error);
    ...
    <div className="space-y-6 px-5">
      <Breadcrumbs
        items={[
          ...
          { label: "Países", href: "/es/paises" },
          { label: countryName },
        ]}
        currentPath={`/es/paises/${slug}`}
      />
      <h1 className="text-3xl font-bold tracking-tight">Pinturas de {countryName}</h1>
      <p className="max-w-3xl text-neutral-700">
        Explora obras de arte de dominio público de {countryName} — pinturas, ilustraciones y piezas de museo, gratis
        para descargar.
      </p>
