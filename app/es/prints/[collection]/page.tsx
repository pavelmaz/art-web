...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const { name, matched } = await loadCollectionWorks("print", collection, "es");
  if (!name) return {};
  ...
  return {
    ...
    description,
    alternates: collectionAlternates("print", collection, "es"),
...
export default async function Page({ params }: Props) {
  const { collection } = await params;
  return <CollectionDetailPage hub="print" slugParam={collection} locale="es" />;
}
