...
export async function generateMetadata({ params, searchParams }: GenrePageProps): Promise<Metadata> {
  ...
  const title = `${displayName} — 무료 그림 | Fine Art Free`;
  const description =
    genre.description_ko?.trim() ||
  ...
  return {
    ...
    description,
    alternates: paginatedAlternates(`${localePath("ko", "genres")}/${linkSlug}`, page, buildGenreLanguageAlternates(genre.slug)),
...
export default async function GenrePage({ params, searchParams }: GenrePageProps) {
  ...
  const displayName = genre.name_ko?.trim() || genre.name;
  const intro = genre.description_ko?.trim() || genre.description?.trim() || null;
  const linkSlug = genre.slug_ko?.trim() || genre.slug;
  ...
  const { data, count, error } = await supabase
    .from("artworks")
    .select(
      "id, title, title_ko, slug, artist_display, image_id, url, museum, style_title, genre_title, score, alt_text",
      { count: "exact" }
    )
    .eq("genre_title", genre.name)
  ...
  if (error) {
    console.error("[GenrePage/ko]", slug, genre.name, error);
  ...
  return (
    <div className="space-y-6 px-5">
      ...
      <Breadcrumbs
        items={[
          { label: "Home", href: "/ko" },
          { label: "Genres", href: "/ko/genres" },
          { label: displayName },
        ]}
        currentPath={`/ko/genres/${linkSlug}`}
      />
      <h1 className="text-3xl font-bold tracking-tight">{displayName} 그림</h1>
      {intro ? (
        <div className="max-w-3xl mb-8 text-sm leading-relaxed text-[#4a4a4a]">
          <p>{intro}</p>
        </div>
      ) : null}
      <ArtworkGrid artworks={uniqueArtworks} basePath="/ko" />
      <Pagination
        currentPage={page}
        totalPages={pagesOrNotFound(page, totalCount || artworks.length)}
        basePath={`/ko/genres/${linkSlug}`}
      />
    </div>
  );
}
