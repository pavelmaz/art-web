...
export const metadata: Metadata = {
  title: "Fine Art Free — 40万件以上のパブリックドメイン美術を無料ダウンロード",
  description:
    "世界の美術館所蔵の古典絵画・版画・イラストを40万件以上、無料で高解像度ダウンロード。個人・商用利用可。",
  openGraph: {
    title: "Fine Art Free — 40万件以上のパブリックドメイン美術を無料ダウンロード",
    description:
      "世界の美術館所蔵の古典絵画・版画・イラストを40万件以上、無料で高解像度ダウンロード。個人・商用利用可。",
  ...
  const genreImages = await Promise.all(
    browseGenres.map(async (genre) => {
      const { data } = await supabase
        ...
        .limit(1);
      const row = data?.[0];
      const slug = genre.slug?.trim() || genre.slug;
      const label = genre.name_ja?.trim() || genre.name;
      return {
        slug,
        label,
    ...
    <div>
      ...
      <section className="relative w-full bg-[#1a1a1a] py-24">
        ...
        <div className="relative z-10 mx-auto max-w-7xl px-5 text-left">
          ...
          <form action="/ja/search" method="get" className="mt-14 max-w-3xl md:mt-16">
            <div className="flex items-center border-b border-white">
              ...
              />
              <button type="submit" className="px-2 text-3xl text-white hover:text-white" aria-label={t.search}>
                ⌕
              </button>
            </div>
          </form>
            ...
            <div className="flex gap-3 overflow-x-auto pb-2">
              {safeFeaturedArtists.map((artist) => {
                ...
                return (
                  <Link
                    key={artist.name}
                    href={`/ja/artists/${slugify(artist.name)}`}
                  ...
                  >
                    ...
                    <div className="absolute bottom-0 left-0 p-3">
                      ...
                      <p className="text-[11px] text-white/90 drop-shadow-sm">{artist.count} 点</p>
                    </div>
                  </Link>
                );
              })}
