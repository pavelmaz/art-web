...
export async function generateMetadata({ params, searchParams }: StylePageProps): Promise<Metadata> {
  ...
  const title = `${displayName} — Stili gratuiti | Fine Art Free`;
  const description =
  ...
  return {
    ...
    description,
    alternates: paginatedAlternates(`${localePath("it", "styles")}/${linkSlug}`, page, buildStyleLanguageAlternates(style?.slug || slug)),
...
export default async function StylePage({ params, searchParams }: StylePageProps) {
  ...
  return (
    <div className="space-y-6 px-5">
      ...
      />
      <h1 className="text-3xl font-bold tracking-tight">Stile {displayName}</h1>
      {intro ? (
