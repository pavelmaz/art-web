...
export async function generateMetadata({ params, searchParams }: GenrePageProps): Promise<Metadata> {
  ...
  const title = `${displayName} — Dipinti gratuiti | Fine Art Free`;
  const description =
    genre.description_it?.trim() ||
    genre.description?.trim() ||
    `Scopri ${totalCount} opere di ${displayName} in alta risoluzione. Dominio pubblico, gratuito.`;
  ...
  return {
    ...
    description,
    alternates: paginatedAlternates(`${localePath("it", "genres")}/${linkSlug}`, page, buildGenreLanguageAlternates(genre.slug)),
...
export default async function GenrePage({ params, searchParams }: GenrePageProps) {
  ...
  const displayName = genre.name_it?.trim() || genre.name;
  const intro = genre.description_it?.trim() || genre.description?.trim() || null;
  const linkSlug = genre.slug_it?.trim() || genre.slug;
  ...
  if (error) {
    console.error("[GenrePage/it]", slug, genre.name, error);
  ...
  return (
    <div className="space-y-6 px-5">
      <CollectionPageJsonLd
        name={`Dipinti di ${displayName}`}
        path={`/it/genres/${linkSlug}`}
        description={intro}
      ...
      <Breadcrumbs
        items={[
          { label: "Home", href: "/it" },
          { label: "Genres", href: "/it/genres" },
          { label: displayName },
        ]}
        currentPath={`/it/genres/${linkSlug}`}
      />
      <h1 className="text-3xl font-bold tracking-tight">Dipinti di {displayName}</h1>
      {intro ? (
