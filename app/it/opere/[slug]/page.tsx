...
async function resolveGenreHubLinkLocal(
  ...
  return { href: `/it/generi/${linkSlug}`, label };
}
...
async function resolveStyleHubLinkLocal(
  ...
  const linkSlug = row.slug_it?.trim() || row.slug;
  const label = row.name_it?.trim() || row.name;
  return { href: `/it/stili/${linkSlug}`, label };
}
  ...
  const jsonLd = {
    ...
    itemListElement: [
      ...
      },
      {
        ...
        item: category ? `${siteUrl}${category.href}` : `${siteUrl}/it/opere`,
      },
      {
        "@type": "ListItem",
        position: 3,
        name: artwork.title,
        item: `${siteUrl}/it/opere/${artwork.slug}`,
      },
    ],
  };
...
async function getArtworkBySlug(slug: string): Promise<ArtworkRow | null> {
  const selectColumns =
    "id, slug, title, artist_display, url, image_id, museum, style_title, genre_title, medium_display, date_display, dimensions, description, description_it";
...
export default async function ArtworkDetailPageIt({ params }: ArtworkPageProps) {
  ...
  const breadcrumbItems = [
    ...
    ...(artwork.artist_display?.trim()
      ? [{ label: artwork.artist_display.trim(), href: `/it/artisti/${slugify(artwork.artist_display)}` }]
      : []),
    { label: artwork.title },
  ];
  ...
  return (
    <article className="bg-[#faf9f7] py-8">
      <div className="mx-auto max-w-7xl px-5">
        <ArtworkJsonLd artwork={artwork} pageUrl={`https://fineartfree.com${artworkDetailPath("it", artwork.slug)}`} inLanguage="it" />
        <BreadcrumbJsonLd artwork={artwork} category={category} />

        <div className="flex flex-col gap-8 lg:flex-row lg:items-start">
          <ArtworkInsightsProvider artwork={artwork} locale="it">
            <div className="flex-1 space-y-4">
              ...
              <Breadcrumbs items={breadcrumbItems} currentPath={`/it/opere/${artwork.slug}`} includeJsonLd={false} />
            </div>
          </ArtworkInsightsProvider>
          ...
          <aside className="w-full lg:w-80">
            <div className="space-y-4 rounded-2xl bg-[#f5f5f5] p-5 lg:sticky lg:top-6">
              <div>
                ...
                {artistSlug ? (
                  <Link href={`/it/artisti/${artistSlug}`} className="text-sm text-[#6b6b6b] hover:text-[#1a1a1a]">
                    {artist}
                  </Link>
                ) : (
              ...
              <div className="space-y-3 rounded-lg bg-[#eceff3] p-3">
                ...
                {artwork.museum?.trim() ? (
                  <div>
                    ...
                    <Link
                      href={`/it/musei/${slugify(artwork.museum)}`}
        ...
        {descriptionText?.trim() ? (
          <section className="mt-10">
            ...
            {artistSlug && relatedArtworks.length === 0 ? (
              <SectionCtaLink href={`/it/artisti/${artistSlug}`}>{t.browseAll}</SectionCtaLink>
            ) : null}
          </section>
        ) : null}
          ...
          <section className="mt-10">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
              {relatedArtworks.map((item) => (
                <a key={item.id} href={`/it/opere/${item.slug}`} className="group block">
