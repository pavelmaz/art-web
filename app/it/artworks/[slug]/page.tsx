...
export default async function ArtworkDetailPageIt({ params }: ArtworkPageProps) {
  ...
  if (artwork.artist_display?.trim()) {
    const relatedQuery = await supabase
      .from("artworks")
      .select("id, title, title_it, slug, artist_display, image_id, url, museum, style_title, genre_title, score, alt_text")
      .eq("artist_display", artwork.artist_display)
  ...
  if (artwork.genre_title) {
    const { data: genreData } = await supabase
      .from("artworks")
      .select("id, title, title_it, slug, artist_display, image_id, url, museum, style_title, genre_title, alt_text")
          ...
          <aside className="order-2 w-full lg:w-96">
            <div className="glass-surface space-y-4 rounded-2xl p-5 lg:sticky lg:top-6">
              ...
              <div className="space-y-3">
                ...
                <ProDownloadRow locale="it" downloadHref={maxDownloadHref} filename={artwork.slug} glass maxDims={artworkMaxSpecs(artwork)} maxSize={artworkMaxSize(artwork)} />
              </div>
              ...
              <div className="space-y-3 rounded-lg glass-inset p-3">
                {artwork.style_title?.trim() ? (
                  <div>
                    ...
                    <Link
                      href={styleLink?.href ?? `/it/styles/${slugify(artwork.style_title)}`}
                ...
                {artwork.medium_display?.trim() ? (
                  <div>
                    ...
                    <p className="text-sm text-[#1a1a1a]">{localizeMedium(artwork.medium_display, "it")}</p>
                  </div>
                ) : null}
                    ...
                    <div className="flex justify-center">
                      <div className="relative w-fit max-w-full">
                        <ArtworkZoomImage
                          ...
                          alt={localeTranslation?.alt_text || localizeAltText(artwork, "it")}
                        />
                        <ArtworkInsightsOverlay />
                      </div>
                    </div>
