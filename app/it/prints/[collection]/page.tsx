...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const { name, matched } = await loadCollectionWorks("print", collection, "it");
  if (!name) return {};
  ...
  return {
    ...
    description,
    alternates: collectionAlternates("print", collection, "it"),
...
export default async function Page({ params }: Props) {
  const { collection } = await params;
  return <CollectionDetailPage hub="print" slugParam={collection} locale="it" />;
}
