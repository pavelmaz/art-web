...
async function getArtworkBySlug(slug: string): Promise<ArtworkRow | null> {
  const selectColumns =
    "id, slug, title, artist_display, url, image_id, museum, style_title, genre_title, medium_display, date_display, dimensions, description, description_pt, title_pt, img_width, img_height, orig_bytes, std_bytes";
  ...
  if (!primary.error && primary.data) {
    ...
    row.title = row.title_pt?.trim() || row.title;
    return row;
  }
...
export default async function ArtworkDetailPagePt({ params }: ArtworkPageProps) {
  ...
  if (artwork.genre_title) {
    const { data: genreData } = await supabase
      .from("artworks")
      .select("id, title, title_pt, slug, artist_display, image_id, url, museum, style_title, genre_title, alt_text")
  ...
  return (
    <article className="py-8">
      ...
      <div className="mx-auto max-w-7xl px-5">
        <ArtworkJsonLd
          artwork={{ ...artwork, description: artwork.description_pt || artwork.description }}
          pageUrl={absoluteArtworkUrl("pt", artwork.slug)}
          inLanguage="pt"
        />
          ...
          <aside className="order-2 w-full lg:w-96">
            <div className="glass-surface space-y-4 rounded-2xl p-5 lg:sticky lg:top-6">
              <div>
                ...
                <ArtistChip
                  name={artist}
                  href={artistSlug ? `/pt/artistas/${artistSlug}` : null}
              ...
              <div className="mt-4">
                <ArtworkActions artworkId={artwork.id} locale="pt" />
              </div>
              ...
              <div className="space-y-3">
                ...
                <ProDownloadRow locale="pt" downloadHref={maxDownloadHref} filename={artwork.slug} glass maxDims={artworkMaxSpecs(artwork)} maxSize={artworkMaxSize(artwork)} />
              </div>
              ...
              <div className="space-y-3 rounded-lg glass-inset p-3">
                {artwork.style_title?.trim() ? (
                  <div>
                    ...
                    <Link
                      href={`/pt/estilos/${slugify(artwork.style_title)}`}
                ...
                {artwork.medium_display?.trim() ? (
                  <div>
                    ...
                    <p className="text-sm text-[#1a1a1a]">{localizeMedium(artwork.medium_display, "pt")}</p>
                  </div>
                ) : null}
              ...
              <Link
                href="/pt/imagens-de-dominio-publico-para-uso-comercial"
              ...
              >
                ...
                Imagens de domínio público para uso comercial
              </Link>
            </div>
          </aside>
                    ...
                    <div className="flex justify-center">
                      <div className="relative w-fit max-w-full">
                        <ArtworkZoomImage
                          ...
                          alt={ptTranslation?.alt_text || localizeAltText(artwork, "pt")}
                        />
                        <ArtworkInsightsOverlay />
                      </div>
                    </div>
