...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const { name, matched } = await loadCollectionWorks("book-illustration", collection, "pt");
  if (!name) return {};
  ...
  return {
    ...
    description,
    alternates: collectionAlternates("book-illustration", collection, "pt"),
...
export default async function Page({ params }: Props) {
  const { collection } = await params;
  return <CollectionDetailPage hub="book-illustration" slugParam={collection} locale="pt" />;
}
