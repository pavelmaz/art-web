...
export const metadata: Metadata = {
  title: "Fine Art Free — Baixe 500.000+ Obras de Arte de Domínio Público",
  description:
    "Explore e baixe 500.000+ pinturas clássicas grátis. Arte de domínio público dos melhores museus do mundo. Grátis para uso pessoal e comercial.",
  openGraph: {
    title: "Fine Art Free — Baixe 500.000+ Obras de Arte de Domínio Público",
    description:
      "Explore e baixe 500.000+ pinturas clássicas grátis. Arte de domínio público dos melhores museus do mundo. Grátis para uso pessoal e comercial.",
...
export default async function HomePagePt() {
  ...
  const genreImages = await Promise.all(
    browseGenres.map(async (genre) => {
      const { data } = await supabase
        ...
        .limit(1);
      const row = data?.[0];
      const slug = genre.slug_pt?.trim() || genre.slug;
      const label = genre.name_pt?.trim() || genre.name;
      return {
        slug,
        label,
    ...
    <div>
      ...
      <section className="relative w-full bg-[#1a1a1a] py-24">
        ...
        <div className="relative z-10 mx-auto max-w-7xl px-5 text-left">
          ...
          <form action="/pt/buscar" method="get" className="mt-14 max-w-3xl md:mt-16">
            <div className="flex items-center border-b border-white">
              ...
              />
              <button type="submit" className="px-2 text-3xl text-white hover:text-white" aria-label="Pesquisar">
                ⌕
              </button>
            </div>
          </form>
            ...
            <div className="flex gap-3 overflow-x-auto pb-2">
              {safeFeaturedArtists.map((artist) => {
                ...
                return (
                  <Link
                    key={artist.name}
                    href={`/pt/artistas/${slugify(artist.name)}`}
