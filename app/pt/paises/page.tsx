  ...
  return (
    <div className="space-y-8 px-5">
      <div>
        ...
        <p className="mb-8 text-sm text-[#6b6b6b]">Explorar obras de arte por país de origem</p>
      </div>

      {paginated.length ? (
        <div className="grid grid-cols-2 gap-4 md:grid-cols-4 lg:grid-cols-5">
          {paginated.map((item) => (
            <Link key={item.slug} href={`/pt/paises/${item.slug}`}>
        ...
        <p className="text-sm text-[#6b6b6b]">Nenhum país encontrado.</p>
      )}

      <Pagination currentPage={page} totalPages={totalPages} basePath="/pt/paises" />
    </div>
  );
}
