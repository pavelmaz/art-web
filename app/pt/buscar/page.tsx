...
export default async function SearchPage({ searchParams }: SearchPageProps) {
  ...
  if (!q) {
    return (
      <div className="mx-auto max-w-7xl space-y-4 px-5 py-10">
        <h1 className="text-2xl font-semibold text-[#1a1a1a]">Busca</h1>
        <p className="text-sm text-[#6b6b6b]">Digite uma palavra-chave na barra de pesquisa para ver resultados.</p>
      </div>
    );
  }
  ...
  if (error && !rows.length) {
    return (
      <div className="mx-auto max-w-7xl space-y-4 px-5 py-10">
        <h1 className="text-2xl font-semibold text-[#1a1a1a]">Busca</h1>
        <p className="text-sm text-[#6b6b6b]">Erro ao carregar os resultados da pesquisa.</p>
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-8 px-5 py-10">
      <SearchAnalytics query={q} locale="pt" results={artworks.length} />
      <div className="space-y-2">
        <h1 className="text-3xl font-semibold text-[#1a1a1a]">Resultados para &quot;{q}&quot;</h1>
      </div>
      ...
      <section className="space-y-4">
        ...
        {artworks.length ? (
          ...
          <p className="text-sm text-[#6b6b6b]">Nenhum resultado encontrado</p>
        )}
      </section>
        ...
        <section className="space-y-4">
          ...
          <div className="flex flex-wrap gap-2">
            {matchingTags.map((tag) => (
              <Link
                key={tag}
                href={`/pt/artworks?tag=${encodeURIComponent(tag)}`}
