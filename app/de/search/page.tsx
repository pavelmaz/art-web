...
export default async function SearchPage({ searchParams }: SearchPageProps) {
  ...
  if (error && !rows.length) {
    return (
      <div className="mx-auto max-w-7xl space-y-4 px-5 py-10">
        <h1 className="text-2xl font-semibold text-[#1a1a1a]">Suche</h1>
        <p className="text-sm text-[#6b6b6b]">Fehler beim Laden der Suchergebnisse.</p>
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-8 px-5 py-10">
      <SearchAnalytics query={q} locale="de" results={artworks.length} />
      <div className="space-y-2">
        <h1 className="text-3xl font-semibold text-[#1a1a1a]">Ergebnisse für &quot;{q}&quot;</h1>
      </div>
      ...
      <section className="space-y-4">
        ...
        {artworks.length ? (
          ...
          <p className="text-sm text-[#6b6b6b]">Keine Ergebnisse</p>
        )}
      </section>

      {matchingTags.length ? (
        <section className="space-y-4">
          <h2 className="text-xl font-semibold text-[#1a1a1a]">Themen</h2>
      ...
      <section className="space-y-4">
        <h2 className="text-xl font-semibold text-[#1a1a1a]">Künstler ({artistResults.length})</h2>
