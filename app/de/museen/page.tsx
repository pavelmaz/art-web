...
export const metadata: Metadata = {
  ...
  alternates: {
    canonical: canonicalHubUrl("de", "museums"),
...
export default async function MuseumsPage({ searchParams }: MuseumsPageProps) {
  ...
  const items = aggregated.map((row) => ({
    name: row.display,
    href: `/de/museen/${slugify(row.display)}`,
    count: row.count,
  ...
  return (
    <div className="space-y-8 px-5">
      <div>
        ...
        <p className="mb-8 text-sm text-[#6b6b6b]">Explorar werke de arte por museo</p>
      </div>

      {paginated.length ? <BrowseHubGrid items={paginated} /> : <p className="text-sm text-[#6b6b6b]">No se encontraron museen.</p>}

      <Pagination currentPage={page} totalPages={totalPages} basePath="/de/museen" />
    </div>
  );
}
