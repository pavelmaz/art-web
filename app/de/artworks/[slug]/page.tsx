...
// the nightly revalidate-changed step), not on a timer; scrapers re-crawl the
// 5M-URL catalogue weekly, and a weekly expiry made every pass a paid render.
// 90 days is only a safety net for drift (artist counts, related works).
...
type ArtworkRow = {
  ...
  description_ger: string | null;
  title_ger: string | null;
...
async function resolveGenreHubLinkLocal(
  ...
  const { data, error } = await supabase
    .from("genres")
    .select("name, name_de, slug, slug_de")
    .eq("name", name)
    .maybeSingle();
  ...
  const linkSlug = row.slug_de?.trim() || row.slug;
  const label = row.name_de?.trim() || row.name;
  return { href: `/de/genres/${linkSlug}`, label };
}
...
async function resolveStyleHubLinkLocal(
  ...
  const linkSlug = row.slug_de?.trim() || row.slug;
  const label = row.name_de?.trim() || row.name;
  return { href: `/de/styles/${linkSlug}`, label };
}
...
async function resolveCategoryBreadcrumbDe(
  artwork: ArtworkRow
  ...
  const jsonLd = {
    ...
    itemListElement: [
      ...
      },
      {
        "@type": "ListItem",
        position: 2,
        name: category?.label || "Werke",
        item: category ? `${siteUrl}${category.href}` : `${siteUrl}/de/artworks`,
      },
      {
        "@type": "ListItem",
        position: 3,
        name: artwork.title,
        item: `${siteUrl}/de/artworks/${artwork.slug}`,
      },
    ],
  };
...
async function getArtworkBySlug(slug: string): Promise<ArtworkRow | null> {
  const selectColumns = artworkDetailSelectColumns("de");
  ...
  if (!primary.error && primary.data) {
    ...
    row.title = row.title_ger?.trim() || row.title;
    return row;
  }
...
export async function generateMetadata({ params }: ArtworkPageProps): Promise<Metadata> {
  ...
  const description =
    ...
    artwork.description_ger?.slice(0, 200) ||
    `${artwork.title} de ${artist}. Gemeinfreie Kunst kostenlos.`;
...
export default async function ArtworkDetailPageDe({ params }: ArtworkPageProps) {
  ...
  const category = await resolveCategoryBreadcrumbDe(artwork);
  ...
  const breadcrumbItems = [
    ...
    ...(artwork.artist_display?.trim()
      ? [{ label: artwork.artist_display.trim(), href: `/de/artists/${slugify(artwork.artist_display)}` }]
      : []),
    { label: artwork.title },
  ];
  ...
  if (artwork.genre_title) {
    const { data: genreData } = await supabase
      .from("artworks")
      .select("id, title, title_ger, slug, artist_display, image_id, url, museum, style_title, genre_title, alt_text")
  ...
  const descriptionText = artwork.description_ger || artwork.description;
  ...
  return (
    <article className="py-8">
      ...
      <div className="mx-auto max-w-7xl px-5">
        <ArtworkJsonLd artwork={{ ...artwork, description: artwork.description_ger || artwork.description }} pageUrl={`https://fineartfree.com${artworkDetailPath("de", artwork.slug)}`} inLanguage="de" />
          ...
          <aside className="order-2 w-full lg:w-96">
            <div className="glass-surface space-y-4 rounded-2xl p-5 lg:sticky lg:top-6">
              <div>
                ...
                <ArtistChip
                  name={artist}
                  href={artistSlug ? `/de/artists/${artistSlug}` : null}
              ...
              <div className="mt-4">
                <ArtworkActions artworkId={artwork.id} locale="de" />
              </div>
              ...
              <div className="space-y-3">
                ...
                <ProDownloadRow locale="de" downloadHref={maxDownloadHref} filename={artwork.slug} glass maxDims={artworkMaxSpecs(artwork)} maxSize={artworkMaxSize(artwork)} />
              </div>
                  ...
                  <div className="mt-3 rounded-lg bg-white p-4">
                    <p className="leading-relaxed text-[#4a4a4a]">
                      {artistDeathYear != null ? (
                        <>
                          Der Künstler verstarb im Jahr {artistDeathYear}, daher ist dieses Werk in seinem Herkunftsland
                          und in anderen Ländern gemeinfrei, in denen die Schutzfrist die Lebenszeit des Künstlers plus 70
                          Jahre oder weniger beträgt.
                        </>
                      ) : (
                        <>
                          Dieses Werk ist in seinem Herkunftsland und in anderen Ländern gemeinfrei, in denen die
                          Schutzfrist die Lebenszeit des Künstlers plus 70 Jahre oder weniger beträgt.
                        </>
                      )}
                    </p>
                  </div>
              ...
              <div className="space-y-3 rounded-lg glass-inset p-3">
                {artwork.style_title?.trim() ? (
                  <div>
                    ...
                    <Link
                      href={styleLink?.href ?? `/de/styles/${slugify(artwork.style_title)}`}
                ...
                {artwork.medium_display?.trim() ? (
                  <div>
                    ...
                    <p className="text-sm text-[#1a1a1a]">{localizeMedium(artwork.medium_display, "de")}</p>
                  </div>
                ) : null}
                ...
                {artwork.museum?.trim() ? (
                  <div>
                    ...
                    <Link
                      href={`/de/museums/${slugify(artwork.museum)}`}
                      className="text-sm text-[#1a1a1a] underline"
                    >
                      {artwork.museum}
                    </Link>
              ...
              <Link
                href="/de/gemeinfreie-bilder-kommerzielle-nutzung"
              ...
              >
                ...
                Gemeinfreie Bilder für kommerzielle Nutzung
              </Link>
            </div>
          </aside>

          <ArtworkInsightsProvider artwork={artwork} locale="de">
                    ...
                    <div className="flex justify-center">
                      <div className="relative w-fit max-w-full">
                        <ArtworkZoomImage
                          ...
                          alt={localeTranslation?.alt_text || localizeAltText(artwork, "de")}
                        />
                        <ArtworkInsightsOverlay />
                      </div>
                    </div>
          ...
          <section className="mt-10">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
              {relatedArtworks.map((item) => (
                <a key={item.id} href={`/de/artworks/${item.slug}`} className="group block">
        ...
        {relatedByGenre.length > 0 && artwork.genre_title ? (
          <section className="mt-12">
            <h2 className="mb-4 text-base font-semibold">Mehr Kunst von {genreHubLink?.label ?? artwork.genre_title}</h2>
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
