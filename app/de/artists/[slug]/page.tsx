...
export async function generateMetadata({ params }: ArtistPageProps): Promise<Metadata> {
  ...
  const artist = await getArtistProfileBySlug(slug, "de");
...
export default async function ArtistPage({ params, searchParams }: ArtistPageProps) {
  ...
  const { data, error } = await supabase
    .from("artworks")
    .select(
      "id, title, title_ger, slug, artist_display, image_id, url, museum, style_title, genre_title, score, alt_text",
    )
  ...
  if (error) {
    console.error("[ArtistPage/de]", slug, artistName, error);
  ...
  const bio = getArtistBioForLocale(artist, "de");
