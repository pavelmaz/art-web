...
export async function generateMetadata({ params, searchParams }: StylePageProps): Promise<Metadata> {
  ...
  const displayName = style?.name_zh?.trim() || englishName;
  ...
  return {
    ...
    description,
    alternates: paginatedAlternates(`${localePath("zh", "styles")}/${linkSlug}`, page, buildStyleLanguageAlternates(style?.slug || slug)),
...
export default async function StylePage({ params, searchParams }: StylePageProps) {
  ...
  const englishName = style?.name ?? unslugifyStyle(slug);
  const displayName = style?.name_zh?.trim() || englishName;
  const intro = style?.description_zh?.trim() || style?.description?.trim() || null;
  const linkSlug = style?.slug_zh?.trim() || style?.slug || slug;
  ...
  if (error) {
    console.error("[StylePage/zh]", slug, englishName, error);
