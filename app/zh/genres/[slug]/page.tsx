...
async function getGenreByLocalizedSlug(slug: string): Promise<GenreRow | null> {
  const cols = "name, name_zh, description, description_zh, slug, slug_zh";
...
export async function generateMetadata({ params, searchParams }: GenrePageProps): Promise<Metadata> {
  ...
  const title = `${displayName} — 免费画作 | Fine Art Free`;
  const description =
    genre.description_zh?.trim() ||
  ...
  return {
    ...
    description,
    alternates: paginatedAlternates(`${localePath("zh", "genres")}/${linkSlug}`, page, buildGenreLanguageAlternates(genre.slug)),
...
export default async function GenrePage({ params, searchParams }: GenrePageProps) {
  ...
  const displayName = genre.name_zh?.trim() || genre.name;
  const intro = genre.description_zh?.trim() || genre.description?.trim() || null;
  const linkSlug = genre.slug_zh?.trim() || genre.slug;
  ...
  const { data, count, error } = await supabase
    .from("artworks")
    .select(
      "id, title, title_ch, slug, artist_display, image_id, url, museum, style_title, genre_title, score, alt_text",
      { count: "exact" }
    )
    .eq("genre_title", genre.name)
  ...
  if (error) {
    console.error("[GenrePage/zh]", slug, genre.name, error);
  ...
  return (
    <div className="space-y-6 px-5">
      ...
      />
      <h1 className="text-3xl font-bold tracking-tight">{displayName}画作</h1>
      {intro ? (
        <div className="max-w-3xl mb-8 text-sm leading-relaxed text-[#4a4a4a]">
          <p>{intro}</p>
        </div>
      ) : null}
      <ArtworkGrid artworks={uniqueArtworks} basePath="/zh" />
      <Pagination
        currentPage={page}
