...
export default async function SearchPage({ searchParams }: SearchPageProps) {
  ...
  if (!q) {
    return (
      <div className="mx-auto max-w-7xl space-y-4 px-5 py-10">
        <h1 className="text-2xl font-semibold text-[#1a1a1a]">搜索</h1>
        <p className="text-sm text-[#6b6b6b]">Zhcribe una palabra clave en la barra de búsqueda para ver resultados.</p>
      </div>
    );
  }
  ...
  if (error && !rows.length) {
    return (
      <div className="mx-auto max-w-7xl space-y-4 px-5 py-10">
        <h1 className="text-2xl font-semibold text-[#1a1a1a]">搜索</h1>
        <p className="text-sm text-[#6b6b6b]">加载搜索结果时出错。</p>
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-8 px-5 py-10">
      <SearchAnalytics query={q} locale="zh" results={artworks.length} />
      <div className="space-y-2">
        <h1 className="text-3xl font-semibold text-[#1a1a1a]">搜索结果 &quot;{q}&quot;</h1>
      </div>

      <section className="space-y-4">
        <h2 className="text-xl font-semibold text-[#1a1a1a]">作品 ({artworks.length})</h2>
        {artworks.length ? (
          <ArtworkGrid artworks={artworks} basePath="/zh" />
        ) : (
          <p className="text-sm text-[#6b6b6b]">未找到结果</p>
        )}
      </section>

      {matchingTags.length ? (
        <section className="space-y-4">
          <h2 className="text-xl font-semibold text-[#1a1a1a]">主题</h2>
          <div className="flex flex-wrap gap-2">
            {matchingTags.map((tag) => (
              <Link
                key={tag}
                href={`/zh/artworks?tag=${encodeURIComponent(tag)}`}
      ...
      <section className="space-y-4">
        <h2 className="text-xl font-semibold text-[#1a1a1a]">艺术家 ({artistResults.length})</h2>
