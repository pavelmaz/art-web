...
async function getArtworkBySlug(slug: string): Promise<ArtworkRow | null> {
  ...
  if (!primary.error && primary.data) {
    ...
    row.title = row.title_ch?.trim() || row.title;
    return row;
  }
...
export async function generateMetadata({ params }: ArtworkPageProps): Promise<Metadata> {
  ...
  const description =
    ...
    artwork.description_ch?.slice(0, 200) ||
    `${artwork.title} de ${artist}. 免费公共领域艺术。`;
  ...
  return {
    ...
    description,
    alternates: {
      canonical: `https://fineartfree.com${artworkDetailPath("zh", slug)}`,
...
export default async function ArtworkDetailPageZh({ params }: ArtworkPageProps) {
  const { slug } = await params;
  const artwork = await getArtworkBySlug(slug);
  const t = getT("zh");
  ...
  const descriptionText = artwork.description_ch || artwork.description;
    ...
    <article className="py-8">
      ...
      <div className="mx-auto max-w-7xl px-5">
        ...
        <div className="flex flex-col gap-8 lg:flex-row lg:items-start">
          <aside className="order-2 w-full lg:w-96">
            <div className="glass-surface space-y-4 rounded-2xl p-5 lg:sticky lg:top-6">
              <div>
                ...
                <ArtistChip
                  name={artist}
                  href={artistSlug ? `/zh/artists/${artistSlug}` : null}
              ...
              <div className="mt-4">
                <ArtworkActions artworkId={artwork.id} locale="zh" />
              </div>
              ...
              <div className="space-y-3">
                ...
                <ProDownloadRow locale="zh" downloadHref={maxDownloadHref} filename={artwork.slug} glass maxDims={artworkMaxSpecs(artwork)} maxSize={artworkMaxSize(artwork)} />
              </div>
                  ...
                  <div className="mt-3 rounded-lg bg-white p-4">
                    <p className="leading-relaxed text-[#4a4a4a]">
                      {artistDeathYear != null ? (
                        <>
                          该艺术家于{artistDeathYear}年去世，因此该作品在其原籍国以及版权期限为艺术家生命加70年或更短的其他国家属于公共领域。
                        </>
                      ) : (
                        <>
                          该作品在其原籍国以及版权期限为艺术家生命加70年或更短的其他国家属于公共领域。
                        </>
                      )}
                    </p>
                  </div>
              ...
              <div className="space-y-3 rounded-lg glass-inset p-3">
                {artwork.style_title?.trim() ? (
                  <div>
                    ...
                    <Link
                      href={styleLink?.href ?? `/zh/styles/${slugify(artwork.style_title)}`}
                ...
                {artwork.medium_display?.trim() ? (
                  <div>
                    ...
                    <p className="text-sm text-[#1a1a1a]">{localizeMedium(artwork.medium_display, "zh")}</p>
                  </div>
                ) : null}
          ...
          <ArtworkInsightsProvider artwork={artwork} locale="zh">
                    ...
                    <div className="flex justify-center">
                      <div className="relative w-fit max-w-full">
                        <ArtworkZoomImage
                          ...
                          alt={localeTranslation?.alt_text || localizeAltText(artwork, "zh")}
                        />
                        <ArtworkInsightsOverlay />
                      </div>
                    </div>
