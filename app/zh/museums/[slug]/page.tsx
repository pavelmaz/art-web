...
export default async function MuseumPage({ params, searchParams }: MuseumPageProps) {
  ...
  const [{ artworks, totalCount, error }, topArtists] = await Promise.all([
    fetchMuseumArtworks(museum.name, from, to, "zh"),
  ...
  return (
    <div className="space-y-8 px-5">
      ...
      <MuseumProfileHeader
        ...
        readMoreLabel="阅读更多"
      />
      <MuseumGuideBanner museumSlug={slug} museumName={museum.name} locale="zh" />
      <MuseumTopArtists artists={topArtists} heading={t.topArtists} locale="zh" />
