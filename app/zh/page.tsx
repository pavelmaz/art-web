  ...
  const genreImages = await Promise.all(
    browseGenres.map(async (genre) => {
      const { data } = await supabase
        ...
        .limit(1);
      const row = data?.[0];
      const slug = getGenreSlugForLocale(genre, "zh");
      const label = getGenreLabelForLocale(genre, "zh");
      return {
        slug,
        label,
    ...
    <div>
      ...
      <section className="relative w-full bg-[#1a1a1a] py-24">
        ...
        <div className="relative z-10 mx-auto max-w-7xl px-5 text-left">
          ...
          <form action="/zh/搜索" method="get" className="mt-14 max-w-3xl md:mt-16">
      ...
      <div className="w-full overflow-x-auto scrollbar-hide bg-[#faf9f7] px-5 py-6">
        <div className="flex gap-3 min-w-max">
          {genreImages.map((genre) => (
            <Link
              key={genre.slug}
              href={`${localePath("zh", "genres")}/${genre.slug}`}
            ...
            <div className="flex gap-3 overflow-x-auto pb-2">
              {safeFeaturedArtists.map((artist) => {
                ...
                return (
                  <Link
                    key={artist.name}
                    href={`/zh/艺术家/${slugify(artist.name)}`}
