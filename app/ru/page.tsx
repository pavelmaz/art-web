...
export const metadata: Metadata = {
  title: { absolute: "Fine Art Free — Скачайте 500 000+ произведений искусства в общественном достоянии" },
  description:
  ...
  const genreImages = await Promise.all(
    browseGenres.map(async (genre) => {
      const { data } = await supabase
        ...
        .limit(1);
      const row = data?.[0];
      const slug = getGenreSlugForLocale(genre, "ru");
      const label = getGenreLabelForLocale(genre, "ru");
      return {
        slug,
        label,
      ...
      <div className="w-full overflow-x-auto scrollbar-hide bg-[#f6f4ee] px-5 py-6">
        <div className="flex gap-3 min-w-max">
          {genreImages.map((genre) => (
            <Link
              key={genre.slug}
              href={`${localePath("ru", "genres")}/${genre.slug}`}
            ...
            <div className="flex gap-3 overflow-x-auto pb-2">
              {safeFeaturedArtists.map((artist) => {
                ...
                return (
                  <Link
                    key={artist.name}
                    href={`/ru/artists/${slugify(artist.name)}`}
