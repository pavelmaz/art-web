...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const { name, matched } = await loadCollectionWorks("book-illustration", collection, "ru");
  if (!name) return {};
  ...
  return {
    ...
    description,
    alternates: collectionAlternates("book-illustration", collection, "ru"),
...
export default async function Page({ params }: Props) {
  const { collection } = await params;
  return <CollectionDetailPage hub="book-illustration" slugParam={collection} locale="ru" />;
}
