...
export async function generateMetadata({ params, searchParams }: GenrePageProps): Promise<Metadata> {
  ...
  const title = `${displayName} — Бесплатные картины | Fine Art Free`;
  const description =
    genre.description_ru?.trim() ||
  ...
  return {
    ...
    description,
    alternates: paginatedAlternates(`${localePath("ru", "genres")}/${linkSlug}`, page, buildGenreLanguageAlternates(genre.slug)),
...
export default async function GenrePage({ params, searchParams }: GenrePageProps) {
  ...
  const displayName = genre.name_ru?.trim() || genre.name;
  const intro = genre.description_ru?.trim() || genre.description?.trim() || null;
  const linkSlug = genre.slug_ru?.trim() || genre.slug;
  ...
  if (error) {
    console.error("[GenrePage/ru]", slug, genre.name, error);
  ...
  return (
    <div className="space-y-6 px-5">
      <CollectionPageJsonLd
        name={`Картины: ${displayName}`}
        path={`/ru/genres/${linkSlug}`}
        description={intro}
      ...
      <Breadcrumbs
        items={[
          { label: "Home", href: "/ru" },
          { label: "Genres", href: "/ru/genres" },
          { label: displayName },
        ]}
        currentPath={`/ru/genres/${linkSlug}`}
      />
      <h1 className="text-3xl font-bold tracking-tight">Картины: {displayName}</h1>
      {intro ? (
        <div className="max-w-3xl mb-8 text-sm leading-relaxed text-[#4a4a4a]">
          <p>{intro}</p>
        </div>
      ) : null}
      <ArtworkGrid artworks={uniqueArtworks} basePath="/ru" />
      <Pagination
        currentPage={page}
        totalPages={pagesOrNotFound(page, totalCount || artworks.length)}
        basePath={`/ru/genres/${linkSlug}`}
      />
    </div>
  );
}
