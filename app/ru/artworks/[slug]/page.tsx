...
async function resolveGenreHubLinkLocal(
  ...
  const { data, error } = await supabase
    .from("genres")
    .select("name, name_ru, slug, slug_ru")
    .eq("name", name)
    .maybeSingle();
  ...
  const linkSlug = row.slug_ru?.trim() || row.slug;
  const label = row.name_ru?.trim() || row.name;
  return { href: `/ru/genres/${linkSlug}`, label };
}
...
async function resolveStyleHubLinkLocal(
  ...
  const linkSlug = row.slug_ru?.trim() || row.slug;
  const label = row.name_ru?.trim() || row.name;
  return { href: `/ru/styles/${linkSlug}`, label };
}
...
async function resolveCategoryBreadcrumbRu(
  artwork: ArtworkRow
  ...
  const jsonLd = {
    ...
    itemListElement: [
      ...
      },
      {
        "@type": "ListItem",
        position: 2,
        name: category?.label || "Произведения",
        item: category ? `${siteUrl}${category.href}` : `${siteUrl}/ru/artworks`,
      },
      {
        "@type": "ListItem",
        position: 3,
        name: artwork.title,
        item: `${siteUrl}/ru/artworks/${artwork.slug}`,
      },
    ],
  };
...
async function getArtworkBySlug(slug: string): Promise<ArtworkRow | null> {
  const selectColumns = artworkDetailSelectColumns("ru");
  ...
  if (!primary.error && primary.data) {
    ...
    row.title = row.title_ru?.trim() || row.title;
    return row;
  }
...
export async function generateMetadata({ params }: ArtworkPageProps): Promise<Metadata> {
  ...
  const description =
    ...
    artwork.description_ru?.slice(0, 200) ||
    `${artwork.title} de ${artist}. Искусство в общественном достоянии бесплатно.`;
  ...
  return {
    ...
    description,
    alternates: {
      canonical: `https://fineartfree.com${artworkDetailPath("ru", slug)}`,
...
export default async function ArtworkDetailPageRu({ params }: ArtworkPageProps) {
  const { slug } = await params;
  const artwork = await getArtworkBySlug(slug);
  const t = getT("ru");
  ...
  const category = await resolveCategoryBreadcrumbRu(artwork);
  ...
  const breadcrumbItems = [
    ...
    ...(artwork.artist_display?.trim()
      ? [{ label: artwork.artist_display.trim(), href: `/ru/artists/${slugify(artwork.artist_display)}` }]
      : []),
    { label: artwork.title },
  ];
  ...
  const descriptionText = artwork.description_ru || artwork.description;
  ...
  return (
    <article className="py-8">
      ...
      <div className="mx-auto max-w-7xl px-5">
        <ArtworkJsonLd artwork={{ ...artwork, description: artwork.description_ru || artwork.description }} pageUrl={`https://fineartfree.com${artworkDetailPath("ru", artwork.slug)}`} inLanguage="ru" />
          ...
          <aside className="order-2 w-full lg:w-96">
            <div className="glass-surface space-y-4 rounded-2xl p-5 lg:sticky lg:top-6">
              <div>
                ...
                <ArtistChip
                  name={artist}
                  href={artistSlug ? `/ru/artists/${artistSlug}` : null}
              ...
              <div className="mt-4">
                <ArtworkActions artworkId={artwork.id} locale="ru" />
              </div>
              ...
              <div className="space-y-3">
                ...
                <ProDownloadRow locale="ru" downloadHref={maxDownloadHref} filename={artwork.slug} glass maxDims={artworkMaxSpecs(artwork)} maxSize={artworkMaxSize(artwork)} />
              </div>
              ...
              <div className="space-y-3 rounded-lg glass-inset p-3">
                {artwork.style_title?.trim() ? (
                  <div>
                    ...
                    <Link
                      href={styleLink?.href ?? `/ru/styles/${slugify(artwork.style_title)}`}
                ...
                {artwork.medium_display?.trim() ? (
                  <div>
                    ...
                    <p className="text-sm text-[#1a1a1a]">{localizeMedium(artwork.medium_display, "ru")}</p>
                  </div>
                ) : null}
                ...
                {artwork.museum?.trim() ? (
                  <div>
                    ...
                    <Link
                      href={`/ru/museums/${slugify(artwork.museum)}`}
                      className="text-sm text-[#1a1a1a] underline"
                    >
                      {artwork.museum}
                    </Link>
              ...
              <Link
                href="/ru/public-domain-images-for-commercial-use"
              ...
              >
                ...
                Изображения без авторских прав для коммерческого использования
              </Link>
            </div>
          </aside>

          <ArtworkInsightsProvider artwork={artwork} locale="ru">
            <div className="order-1 flex-1 space-y-4">
                    ...
                    <div className="flex justify-center">
                      <div className="relative w-fit max-w-full">
                        <ArtworkZoomImage
                          ...
                          alt={localeTranslation?.alt_text || localizeAltText(artwork, "ru")}
                        />
                        <ArtworkInsightsOverlay />
                      </div>
                    </div>
              ...
              <Breadcrumbs items={breadcrumbItems} currentPath={`/ru/artworks/${artwork.slug}`} includeJsonLd={false} />
              ...
              {descriptionText?.trim() ? (
                <section className="max-w-2xl pt-4">
                  ...
                  {artistSlug && relatedArtworks.length === 0 ? (
                    <SectionCtaLink href={`/ru/artists/${artistSlug}`}>{t.browseAll}</SectionCtaLink>
                  ) : null}
                </section>
              ) : null}
            </div>
          ...
          <section className="mt-10">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
              {relatedArtworks.map((item) => (
                <a key={item.id} href={`/ru/artworks/${item.slug}`} className="group block">
        ...
        {relatedByGenre.length > 0 && artwork.genre_title ? (
          <section className="mt-12">
            <h2 className="mb-4 text-base font-semibold">Больше искусства {genreHubLink?.label ?? artwork.genre_title}</h2>
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
