...
export default async function SearchPage({ searchParams }: SearchPageProps) {
  ...
  if (!q) {
    return (
      <div className="mx-auto max-w-7xl space-y-4 px-5 py-10">
        <h1 className="text-2xl font-semibold text-[#1a1a1a]">Поиск</h1>
        <p className="text-sm text-[#6b6b6b]">Rucribe una palabra clave en la barra de búsqueda para ver resultados.</p>
      </div>
    );
  }
  ...
  if (error && !rows.length) {
    return (
      <div className="mx-auto max-w-7xl space-y-4 px-5 py-10">
        <h1 className="text-2xl font-semibold text-[#1a1a1a]">Поиск</h1>
        <p className="text-sm text-[#6b6b6b]">Ошибка загрузки результатов поиска.</p>
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-8 px-5 py-10">
      <SearchAnalytics query={q} locale="ru" results={artworks.length} />
      <div className="space-y-2">
        <h1 className="text-3xl font-semibold text-[#1a1a1a]">Результаты для &quot;{q}&quot;</h1>
      </div>

      <section className="space-y-4">
        <h2 className="text-xl font-semibold text-[#1a1a1a]">Произведения ({artworks.length})</h2>
        {artworks.length ? (
          <ArtworkGrid artworks={artworks} basePath="/ru" />
        ) : (
          <p className="text-sm text-[#6b6b6b]">Результаты не найдены</p>
        )}
      </section>

      {matchingTags.length ? (
        <section className="space-y-4">
          <h2 className="text-xl font-semibold text-[#1a1a1a]">Темы</h2>
          <div className="flex flex-wrap gap-2">
            {matchingTags.map((tag) => (
              <Link
                key={tag}
                href={`/ru/artworks?tag=${encodeURIComponent(tag)}`}
      ...
      <section className="space-y-4">
        <h2 className="text-xl font-semibold text-[#1a1a1a]">Художники ({artistResults.length})</h2>
        {artistResults.length ? (
          <BrowseHubGrid
            items={artistResults.map((artist) => ({
              name: artist.name,
              href: `/ru/artists/${artist.slug}`,
              count: artist.count,
