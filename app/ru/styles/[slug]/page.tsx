...
export async function generateMetadata({ params, searchParams }: StylePageProps): Promise<Metadata> {
  ...
  const displayName = style?.name_ru?.trim() || englishName;
  ...
  const title = `${displayName} — Бесплатные стили | Fine Art Free`;
  const description =
    style?.description_ru?.trim() ||
    style?.description?.trim() ||
    `Изучите ${totalCount} работ ${displayName} в высоком разрешении. Общественное достояние, бесплатно.`;

  const linkSlug = style?.slug_ru?.trim() || style?.slug || slug;

  return {
    title: { absolute: title },
    description,
    alternates: paginatedAlternates(`${localePath("ru", "styles")}/${linkSlug}`, page, buildStyleLanguageAlternates(style?.slug || slug)),
...
export default async function StylePage({ params, searchParams }: StylePageProps) {
  ...
  const englishName = style?.name ?? unslugifyStyle(slug);
  const displayName = style?.name_ru?.trim() || englishName;
  const intro = style?.description_ru?.trim() || style?.description?.trim() || null;
  const linkSlug = style?.slug_ru?.trim() || style?.slug || slug;
  ...
  const { data, count, error } = await supabase
    .from("artworks")
    .select(
      "id, title, title_ru, slug, artist_display, image_id, url, museum, style_title, genre_title, score, alt_text",
      { count: "exact" }
    )
    .eq("style_title", englishName)
  ...
  if (error) {
    console.error("[StylePage/ru]", slug, englishName, error);
  ...
  return (
    <div className="space-y-6 px-5">
      <CollectionPageJsonLd
        name={`Стиль ${displayName}`}
        path={`/ru/styles/${linkSlug}`}
        description={intro}
      ...
      <Breadcrumbs
        items={[
          { label: "Home", href: "/ru" },
          { label: "Styles", href: "/ru/styles" },
          { label: displayName },
        ]}
        currentPath={`/ru/styles/${linkSlug}`}
      />
      <h1 className="text-3xl font-bold tracking-tight">Стиль {displayName}</h1>
      {intro ? (
